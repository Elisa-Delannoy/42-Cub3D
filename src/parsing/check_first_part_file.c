...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   check_first_part_file.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: edelanno <edelanno@student.42perpignan.    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/11 15:56:11 by edelanno          #+#    #+#             */
/*   Updated: 2025/06/11 15:56:12 by edelanno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_one_instruct(t_map *map)
{
	if (map->c_no == 1 && map->c_so == 1 && map->c_we == 1 && map->c_ea == 1
		...
		|| map->c_f > 1 || map->c_c > 1 || map->c_d > 1 || map->c_x > 1)
		return (1);
	return (2);
}
	...
	while (var->map->tab_file[*i])
	{
		...
		while (var->map->tab_file[*i][j])
		{
			...
			if (var->map->tab_file[*i][j] == '1')
				return (0);
			else if (var->map->tab_file[*i][j]
				&& var->map->tab_file[*i][j] != ' '
