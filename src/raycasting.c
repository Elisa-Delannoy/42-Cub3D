...
t_point	raycating_horizontal(t_var *var, t_cast *cast, float type)
{
	...
	if (sin(cast->ray) < 0)
		...
		cell.y = (floor(var->player->game_y / type) * type) - 1.0f;
	cell.x = var->player->game_x + ((var->player->game_y - cell.y) / cast->tan);
...
t_point	raycating_vertical(t_var *var, t_cast *cast, float type)
{
	...
	if (cos(cast->ray) > 0)
		...
		cast->step_x = -type;
	if (cos(cast->ray) > 0)
		cell.x = (floor(var->player->game_x / type) * type) + type;
	else
		cell.x = (floor(var->player->game_x / type) * type) - 1.0f;
	cell.y = var->player->game_y + ((var->player->game_x - cell.x) * cast->tan);
...
{
	if (fabs(cast->h.x - var->player->game_x) < fabs(cast->v.x - var->player->game_x)
